...
>>>
>>> As metioned before (in Fun & Profit gane engine) measured motor performance (whether trhough gyro/camera - or indirectly as chat performance) relates to the state of heat envelope directly - short-term performance is controlled by breathing (main modulator of heat exchange), long-term by foraging. When Don Genaro (from Carlos Castaneda books) was climbing waterfall rocks to show Carlos the tentacles coming from the nivel area - Genaro was atually controlling his breathing (modulating heat envelope), improving balance and coordination. It is risky for sensory-motor system to act at its peak performance due to smallest unnatural distraction (car moise, Carlos suggesting to be careful or being overly critical of "tentacles idea") being a major factor of risk. It is where word can kill, but akin to science nertheless: proper "lab" setup required to verify claims: temporary absense of critical thought for input observation. Critical thought is good for analysis and planning - not for primary observation and measurement, since in such cases, it allows critical thinker to fit data to theory by filtering every signal that does not correspond the theory, fake the experiment.
